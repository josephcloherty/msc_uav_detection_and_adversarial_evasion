function M = phase5b_RebuildManifest(cfg, seedList)
%phase5b_RebuildManifest Reconstruct a batch manifest from what is on disk.
%
%   M = phase5b_RebuildManifest()            uses phase5b_Config
%   M = phase5b_RebuildManifest(CFG)
%   M = phase5b_RebuildManifest(CFG, SEEDS)  restrict to these seeds
%
%   The manifest is assembled on the client and written when the batch
%   returns, so a client that dies first loses the provenance record even
%   though the runs are safely on disk.
%
%   Nothing about a run's identity is lost in that case. The generation
%   parameters are deterministic in (scenario, seed) and regenerate from the
%   configuration through phase5b_ScenarioGen, the row count comes from the
%   CSV, and the outcome fields that need the live simulation come from the
%   run's own phase5b_RunInfo sidecar.
%
%   Written as manifest_rebuilt_<stamp>.csv, never over a manifest a batch
%   wrote, and returned as a table.
%
%   See also phase5b_RunBatch, phase5b_RunInfo, phase5b_MergeDataset.

    if nargin < 1 || isempty(cfg), cfg = phase5b_Config(); end
    ...
    assert(~isempty(files), 'phase5b_RebuildManifest:noRuns', ...
    ...
    nInfo = 0;
    for i = 1:n
        [rc, mt] = phase5b_ScenarioGen(cfg, seed(i), scen(i));
    ...
    try
        phase5b_CostModel('record', cfg, M, NaN);
    catch err
        warning('phase5b_RebuildManifest:costRecordFailed', ...
...
%   Deliberately a separate implementation from phase5b_RunBatch's private
...
%writeCsv Same hand-rolled format as the batch manifest.
    fid = fopen(outPath, 'w');
    assert(fid > 0, 'phase5b_RebuildManifest:openFailed', ...
