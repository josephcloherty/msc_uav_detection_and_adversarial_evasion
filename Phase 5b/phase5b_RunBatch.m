function manifest = phase5b_RunBatch(cfg)
%phase5b_RunBatch Run the whole batch in parallel and write a manifest.
%
%   manifest = phase5b_RunBatch()      uses phase5b_Config()
%   manifest = phase5b_RunBatch(CFG)   uses a modified configuration
%
%   A run is one seed. Scenario is a function of the seed value, resolved by
%   phase5b_ScenarioFor, so a run's identity does not depend on the shape of
%   the batch it was enumerated in.
%
%   There is no queue: the batch is capped at the pool size and every run
%   starts at once, so a pool smaller than the seed range shortens the batch
%   rather than deferring the remainder. The dropped seeds are named.
    ...
    metas   = cell(1, nRuns);
    for i = 1:nRuns
        [runCfgs{i}, metas{i}] = phase5b_ScenarioGen(cfg, seeds(i), scenarios(i));
    ...
    fprintf('Phase 5b batch: %d run(s) enumerated, %d to execute, %d complete on disk.\n', ...
    ...
    % Nothing queues, so a run beyond the pool is not run at all.
    if nRuns > pool.NumWorkers
        ...
        fprintf(['  Dropped seed(s): %s. Re-run with cfg.batch.seedRange set ' ...
            'to them, or raise cfg.batch.numWorkers to match.\n'], ...
    ...
    elseif nRuns < pool.NumWorkers
        fprintf(['Note: %d run(s) on %d worker(s); %d worker(s) will sit ' ...
            'idle.\n'], nRuns, pool.NumWorkers, pool.NumWorkers - nRuns);
    end
    ...
    kInfo = phase5b_CostModel('effective', cfg, pool.NumWorkers);
    priorWall = kInfo.k * cfg.window.simulationTime * nGNBv .* nUEv;
    reportCostModel(kInfo, priorWall, todo, pool.NumWorkers);

    % Resolved against dataDir when it carries no path of its own.
    pcfg = cfg.progress;
    ...
    prog = phase5b_Progress(labels, pcfg, ...
    ...
    parfor (i = 1:nRuns, pool.NumWorkers)
        ...
        if todo(i)
            try
                rec.summary = phase5b_Pipeline(rc);
                rec.status  = "ok";
            catch err
                rec.status = "failed";
    ...
    phase5b_CostModel('record', cfg, manifest, pool.NumWorkers);
    kAfter = phase5b_CostModel('effective', cfg, pool.NumWorkers);
    if kAfter.n > 0
    ...
    if nOK > 0
        fprintf('Merge the per-run CSVs with: phase5b_MergeDataset\n');
    end
end
...
%   jobs and retry, then halve the worker count and repeat.
    ...
    error('phase5b_RunBatch:poolStartFailed', ...
...
%   Deliberately not a fall back to serial, which would look like a hang.
    ...
    if ~(hasLicence && hasInstall)
        error('phase5b_RunBatch:noParallelToolbox', ...
            ['The Parallel Computing Toolbox is required (licence: %d, ' ...
             'installed: %d). Install or license it, or run a single ' ...
             'scenario directly with phase5b_Pipeline(phase5b_ScenarioGen(' ...
             'phase5b_Config, seed, "UMa")).'], hasLicence, hasInstall);
    end
end
...
%   phase5b_CostModel divides the wall time by them.
    n = numel(runCfgs);
    M = table();
    M.runIdx        = (1:n)';
...
%   vary with the release.
    fid = fopen(outPath, 'w');
    assert(fid > 0, 'phase5b_RunBatch:manifestOpenFailed', ...
...
%quoteField Wrap a text field in quotes when it needs it.
    % Held in a variable: a literal quote next to a closing bracket reads
    % ambiguously against MATLAB's transpose syntax.
    q = '"';
    s = strrep(s, sprintf('\n'), ' ');
