function summary = phase5b_Pipeline(cfg)
%phase5b_Pipeline Headless single-run simulation pipeline.
%
%   SUMMARY = phase5b_Pipeline(CFG) runs one scenario end to end and returns a
%   small plain-data summary, where CFG comes from phase5b_ScenarioGen.
%
...
%     - the replay written to a .mat bundle instead of shown, and only for
%       the seeds named in cfg.replay.saveSeeds
%     - plain data returned, so nothing large crosses back from a worker
%     - a phase5b_PowerTap on the uplink packet path, which is what makes
%       RSRP, RSSI and RSRQ available at all
%     - a runinfo sidecar written from the worker as the run finishes
%
%   Same seed regenerates the CSV byte for byte.
    ...
    rntiMap = phase5b_ResolveRNTI(cfg, gNBs, UEs, anchorIdx, printMap);
    ...
    % Absolute received power for RSRP, RSSI and RSRQ. SINR is a ratio, so
    % without this tap the three columns cannot exist.
    numRB = resolveNumRB(gNBs(1), r);
    powerTap = phase5b_PowerTap([gNBs.ID], [UEs.ID], r.gnbReceiveGain);
    ...
    for u = 1:numsUE
        ...
        managers{u}.powerTap = powerTap;
        managers{u}.numRB    = numRB;
    ...
    replayPath = "";
    if isfield(cfg, 'replay') && cfg.replay.save && replayWantedFor(cfg)
    ...
    % Written from the worker the moment the run finishes, so a client that
    % dies before the batch returns costs no provenance.
    try
        phase5b_RunInfo('write', cfg, summary);
    catch err
        warning('phase5b_Pipeline:runInfoFailed', ...
...
%   Called once after run() returns, so every log is complete.
    ...
    csvPath = phase5b_WriteFeatureCSV(T, cfg);
end

function tf = replayWantedFor(cfg)
%replayWantedFor Whether this run writes a replay bundle.
%   cfg.replay.saveSeeds restricts saving to a named set of seeds. Empty or
%   absent keeps the Phase 5 behaviour of one bundle per run. The bundles are
%   the largest artefact a run produces, so a full sweep only needs enough of
%   them to inspect the mechanism.
    tf = true;
    if ~isfield(cfg.replay, 'saveSeeds') || isempty(cfg.replay.saveSeeds)
        return;
    end
    tf = ismember(cfg.seed, double(cfg.replay.saveSeeds(:))');
end

function n = resolveNumRB(gNB, radio)
%resolveNumRB Measurement bandwidth in resource blocks.
%   Read from the node where the release exposes it, and computed from the
%   TS 38.104 Table 5.3.2-1 transmission bandwidth configuration otherwise.
    n = NaN;
    try
        n = double(gNB.NumResourceBlocks);
    catch
        n = NaN;                    % property absent on this release
    end
    if ~isnan(n) && n >= 1, return; end

    bw  = radio.channelBandwidth / 1e6;
    scs = radio.subcarrierSpacing / 1e3;
    tbl = containers.Map( ...
        {'5_15','10_15','15_15','20_15','25_15','30_15','40_15','50_15', ...
         '5_30','10_30','15_30','20_30','25_30','30_30','40_30','50_30', ...
         '60_30','80_30','90_30','100_30'}, ...
        {25, 52, 79, 106, 133, 160, 216, 270, ...
         11, 24, 38,  51,  65,  78, 106, 133, 162, 217, 245, 273});
    key = sprintf('%d_%d', round(bw), round(scs));
    assert(isKey(tbl, key), 'phase5b_Pipeline:unknownBandwidth', ...
        ['No resource block count for %g MHz at %g kHz SCS, and the node ' ...
         'does not expose one. RSRP, RSSI and RSRQ need the measurement ' ...
         'bandwidth, so this fails now rather than writing three NaN ' ...
         'columns after a full run.'], bw, scs);
    n = tbl(key);
end
...
%   caught here instead of silently leaving the run on the class defaults.
    ...
    for k = 1:numel(f)
        assert(isprop(mgr, f{k}), 'phase5b_Pipeline:unknownHandoverField', ...
            ['cfg.handover.%s is not a handoverManager property. Fix the ' ...
             'name in phase5b_Config, or the run would silently keep the ' ...
             'class default.'], f{k});
        assert(~any(strcmp(f{k}, ctorOnly)), ...
            'phase5b_Pipeline:constructionTimeHandoverField', ...
...
%   applied, and only reached when cfg.srs.periodicitySlots is set.
    ...
    for i = 2:numel(gNBs)
        try
            ...
            error('phase5b_Pipeline:srsOverrideFailed', ...
