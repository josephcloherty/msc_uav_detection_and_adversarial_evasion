function [runCfg, meta] = phase5b_ScenarioGen(base, seed, scenarioName)
%phase5b_ScenarioGen Expand the batch config into one concrete run.
%
%   [RUNCFG, META] = phase5b_ScenarioGen(BASE, SEED, SCENARIONAME) flattens
%   phase5b_Config into the run configuration phase5b_Pipeline expects.
%
...
%   as fixed numbers.
    ...
    assert(isfield(base.scenarios, scenarioName), ...
        'phase5b_ScenarioGen:unknownScenario', ...
        'Scenario "%s" is not defined in phase5b_Config (have: %s).', ...
    ...
    assert(numel(swarmRng) == 2 && swarmRng(1) >= 1 && swarmRng(2) >= swarmRng(1), ...
        'phase5b_ScenarioGen:badSwarmRange', ...
    ...
    if isempty(base.srs.periodicitySlots)
        assert(numUE <= base.srs.maxUEsPerGNB, ...
            'phase5b_ScenarioGen:srsCapacity', ...
    ...
    % Honest runs only: an aerial band below the overlay floor would give
    % aerial links the terrestrial channel.
    assert(sc.aerialAltRange_m(1) > sc.zBoundary, ...
        'phase5b_ScenarioGen:altitudeBelowBoundary', ...
    ...
    switch lower(string(base.mobility.boundsConvention))
        ...
        otherwise
            error('phase5b_ScenarioGen:badBoundsConvention', ...
...
%   degree grid instead would leave the 240-300 sector uncovered at five.
    assert(numGNB >= 1 && numGNB <= 13, 'phase5b_ScenarioGen:numGNB', ...
