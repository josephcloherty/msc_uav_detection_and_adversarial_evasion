function T = phase5b_ExtractWindowedFeatures(managers, cfg, sched, trafficLogs)
%phase5b_ExtractWindowedFeatures Per-UE sliding-window feature extraction.
%
%   T = phase5b_ExtractWindowedFeatures(MANAGERS, CFG, SCHED, TRAFFICLOGS)
%   turns the per-scan handover logs, the per-gNB scheduler logs and the
%   traffic byte series into one feature row per UE per window position.
%
%   Identical to extractWindowedFeatures except for the appended RSRP, RSSI
%   and RSRQ block, so the Phase 5 columns stay an exact prefix and a Phase 5
%   file is a valid truncation. phase5bFeatureSchema holds the column list.
%
%   Every feature comes from network-side observables only, never from UE
%   position, velocity, altitude, identity or the simulator's LOS state. The
%   three power columns are uplink SRS measurements taken at the gNB, so that
%   holds for them too.
%
...
%   A run carried out with no power tap leaves the three power blocks NaN.
    ...
    for k = 1:numel(managers)
        ...
        fl = m.featureLog;
        if isempty(fl)
            warning('phase5b_ExtractWindowedFeatures:emptyLog', ...
        ...
        assert(all([any(cTime) any(cServ) any(cVis) any(cMaxN) ...
            ...
            'phase5b_ExtractWindowedFeatures:schemaMismatch', ...
            'handoverManager.featureNames does not contain the expected columns.');

        % Absent on an archived Phase 5 manager, which leaves the block NaN
        % rather than failing a re-extraction of an older run.
        cRSRP = strcmp(names, 'servingRSRP');
        cRSSI = strcmp(names, 'servingRSSI');
        cRSRQ = strcmp(names, 'servingRSRQ');

        keepRows = fl(:, cTime) >= cfg.settleTime;
        fl = fl(keepRows, :);
        if isempty(fl)
            warning('phase5b_ExtractWindowedFeatures:allSettled', ...
        ...
        if span < cfg.windowLen
            warning('phase5b_ExtractWindowedFeatures:shortRun', ...
        ...
        for s0 = starts
            ...
            [rsrpMean, rsrpVar, rsrpMin, rsrpMax, rsrpTrend] = ...
                powerStats_(fl, in, cRSRP, tw, s0);
            [rssiMean, rssiVar, rssiMin, rssiMax, rssiTrend] = ...
                powerStats_(fl, in, cRSSI, tw, s0);
            [rsrqMean, rsrqVar, rsrqMin, rsrqMax, rsrqTrend] = ...
                powerStats_(fl, in, cRSRQ, tw, s0);
            ...
            assert(numel(newRow) == numel(schema) - 1, ...
                'phase5b_ExtractWindowedFeatures:rowWidthMismatch', ...
                ['Assembled %d numeric values for %d numeric schema ' ...
                 'columns. A column was added to phase5bFeatureSchema ' ...
...
function [mu, va, lo, hi, tr] = powerStats_(fl, in, col, tw, s0)
%powerStats_ mean, variance, extremes and slope of one power column.
%   Returns all NaN when the column is absent or held no observation, which
%   is the whole block on a run with no power tap.
    mu = NaN; va = NaN; lo = NaN; hi = NaN; tr = NaN;
    if ~any(col), return; end
    v = fl(in, col);
    ok = ~isnan(v);
