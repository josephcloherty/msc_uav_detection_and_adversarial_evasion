...
% sets up the same object graph as the pipeline at the smallest size that still
% shows a problem, then dumps what the scheduler logs hold against what the
% extraction expects.
    ...
    %% object graph, identical in structure to the pipeline
    rng(cfg.seed);
    ...
    for g = 1:numGNB
        ...
        if ~isempty(ctx)
            ...
            % a zero count means the release does not surface that
            % quantity, so the feature column will be NaN throughout.
            if size(ctx, 2) >= 9
    ...
    %% verdict, checked per UE against its own anchor gNB
    % a global set test passes when only one UE happens to line up
    fprintf('--- verdict ---------------------------------------------------\n');
    if nCtx == 0 && nGrant == 0
...
% reports any gNB property that exposes the connected UEs, which is the
% authoritative source for the scheduler RNTI.
