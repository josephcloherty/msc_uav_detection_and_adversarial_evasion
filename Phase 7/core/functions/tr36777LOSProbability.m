...
%   including the 100% LOS bands (RMa-AV above 40 m, UMa-AV above 100 m;
%   UMi-AV keeps its formula to 300 m).
%
%   Values:
...
%     d1/d2D + exp(-d2D/p1)*(1 - d1/d2D).
    ...
    switch scenario
        ...
        case "UMA"
            if hUT <= 22.5
                % Table 7.4.2-1 UMa; d2D-out is taken as d2D because every
                % UE in this scenario is outdoor.
                if d2D <= 18
                    p = 1;
                else
                    if hUT <= 13
...
%aerialBandLOS Common aerial-band form from TR 36.777 Table B-1.
    if d2D <= d1
        p = 1;
    else
