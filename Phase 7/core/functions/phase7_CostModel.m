function varargout = phase7_CostModel(action, cfg, varargin)
%phase7_CostModel Measured wall-time cost model for run and batch estimation.
%
%   A run costs roughly
...
%     e = phase7_CostModel('effective', cfg, numWorkers)
%         Struct with k, n, atWorkers, source, kPrior, scaleVsPrior, spread.
%     w = phase7_CostModel('predict', cfg, numGNB, numUE, numWorkers)
...
%   it by hand, and every action falls back to the prior when it is missing.
    ...
    switch string(action)
        ...
        otherwise
            error('phase7_CostModel:unknownAction', ...
...
%recordRuns Append the completed runs of a manifest to the store.
    ...
    try
        ...
        warning('phase7_CostModel:writeFailed', ...
