%% Test K-Nearest Neighbours (KNN)
%  Evaluates the trained KNN on the held-out test set.
%  Aerial is the positive class (the UE class we want to detect).
%  Run train_knn.m first, then this script.

%% Load the trained model and the shared test data
clc;
load('model_knn.mat', 'model');
...
title('KNN - test set confusion matrix');
