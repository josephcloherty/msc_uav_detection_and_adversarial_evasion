%% Test Decision Tree
%  Run train_decision_tree.m first, then this script.
...
assert(~isempty(root), 'test_decision_tree:pathNotSet', ...
    'Run test_all.m, or set the current folder to Phase 6 before running this script.');
load(fullfile(root, 'models', 'model_decision_tree.mat'), 'model');
...
%  A single tree gives one score per leaf, so its ROC is a coarse step function.
