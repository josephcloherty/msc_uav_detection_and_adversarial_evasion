...
%   them; that ratio is recomputed on every message, so the estimate for
%   work not yet started tracks the machine it is running on. The batch ETA
%   then schedules the runs still pending onto the workers as they come
%   free (longest first), rather than extrapolating a mean percentage, which
%   on a pool smaller than the batch reports a finish before the last runs
%   have started.
%
%   DISPLAY MODES
...
%   printed, which is what a log-scraped or headless batch usually wants.
    ...
    properties (Access = private)
        ...
        barH = []                   % single Bar object, one bar per run
        hdr  = []                   % header uilabel
    ...
    methods
        function obj = phase5_Progress(labels, pcfg, opts)
            ...
            %                     original behaviour.
            %   opts.numWorkers   pool size, so the batch estimate can queue
            %                     pending runs onto workers as they free up.
    ...
    methods (Access = private)
        function buildPerRunWindow(obj)
            %buildPerRunWindow One horizontal bar per run in one figure.
            %   A single Bar object holds every run, so a redraw is two
            %   property assignments (lengths and colours) however many
            %   runs there are. Per-run text is carried in the y tick
            %   labels rather than in separate text objects, for the same
            %   reason.
            n = obj.nRuns;
            h = min(120 + 22*n, 820);
            % Wider than the bars need: the y tick labels now carry a per-run
            % ETA as well as the name, percentage and state.
            obj.fig = uifigure('Name', 'Phase 5 batch', ...
                'Position', [100 100 820 h]);
            ...
            obj.ax = uiaxes(g);
            obj.ax.Layout.Row = 2;
            obj.barH = barh(obj.ax, 1:n, zeros(1, n), 0.7, ...
            ...
            obj.refreshTickLabels();
        end

        function refreshTickLabels(obj)
            %refreshTickLabels Per-run text: name, percentage, state, ETA.
            %   The per-run ETA is the point of the readout: a batch-level
            %   percentage says nothing about which run is the one holding
            %   the batch up, and on a pool smaller than the batch the
            %   pending runs need their own estimate too.
            lab = strings(1, obj.nRuns);
            for i = 1:obj.nRuns
                lab(i) = sprintf('%-16s %3.0f%% %-9s %s', obj.labels(i), ...
                    100*obj.frac(i), obj.state(i), obj.runEtaText(i));
            end
            obj.ax.YTickLabel = cellstr(lab);
        end
        ...
        function w = expectedWall(obj, i)
            ...
            %   With no cost model supplied, a run still queued is estimated
        ...
        function t = runEtaText(obj, i)
            ...
            if ~isfinite(e)
                t = "ETA --";
            elseif obj.state(i) == "pending"
                t = "queued ~" + string(fmtDuration(e));
            else
                t = "ETA " + string(fmtDuration(e));
            end
        end
        ...
        function e = eta(obj)
            ...
            %   Running runs each hold a worker until their own ETA expires;
            %   pending runs are then placed longest-first on whichever worker
            %   frees up first, which is the same greedy rule the pool itself
            %   approximates. The batch finishes when the last worker does.
            %
            ...
            %   at once: with twelve runs on six workers it reports the batch
            %   finishing at about half the true time, because the six runs
            %   that have not started yet count as zero per cent rather than
            %   as a full run's work still to do.
            active = ~obj.skipped;
            if ~any(active)
                e = 0;
                return;
            ...
            spill = 0;
            if numel(rem) > nW
                % More runs in flight than workers: cannot happen with parfor,
                % but never let a bookkeeping surprise produce a short ETA.
