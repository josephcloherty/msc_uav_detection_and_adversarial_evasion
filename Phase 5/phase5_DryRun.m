...
%   run outcome columns left empty.
    ...
    if isfield(cfg.batch, 'costModel')
        ...
        % The runner gives every run its own worker and does not queue, so
        % the batch ends with its slowest run, not when the total CPU time
        % has been shared out. Averaging would understate it whenever the
        % runs differ in cost, which they do: UE count varies by about 1.5x.
