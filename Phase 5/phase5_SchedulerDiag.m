function D = phase5_SchedulerDiag(simTime, numGNB, numUE)
%phase5_SchedulerDiag Locate why the CQI and MCS columns come out NaN.
%
%   D = phase5_SchedulerDiag()                  1.5 s, 2 gNB, 3 UE
%   D = phase5_SchedulerDiag(simTime, nGNB, nUE)
%
%   The first end-to-end Phase 5 smoke run produced an entirely empty
%   CQI and MCS block in all three scenarios, while the SRS-derived
%   columns were populated. CQI comes from the scheduler CONTEXT log and
%   the four MCS columns come from the scheduler GRANT log, so two
%   independent paths failed together. That rules out the DL CQI field
%   path left open by the Phase 4 C2 check as the sole cause and points
%   at something shared: either the schedulers logged nothing at all, or
%   the RNTI the extraction uses to find a UE in those logs does not
%   match the RNTI the logs carry, or the anchor gNB index is wrong.
%
%   The handover managers find their UE's SRS events using the same
%   UE.ID + rntiOffset convention and they clearly work, so the SRS event
%   RNTI and the scheduler RNTI may simply be different numbering
%   schemes. The Phase 4 check that passed on this point used a
%   single-gNB probe scenario, where a per-gNB connection index and a
%   node-derived identifier coincide; with seven gNBs they need not.
%
%   This script sets up the same object graph as phase5_Pipeline at the
%   smallest size that can still show the problem, runs it briefly, and
%   dumps what is actually in the logs against what the extraction
%   expects. It writes no CSV and no replay. Keep it small: the smoke run
%   measured roughly 313 times slower than real time at 7 gNB and 3 UE,
%   so the defaults here are chosen to finish in a couple of minutes.
%
%   Read the verdict at the bottom of the output. If the logs are
%   populated but the RNTI sets do not intersect, the fix is the RNTI
%   convention in phase5_Pipeline, not the scheduler class.
    ...
    cfg = phase5_ScenarioGen(base, 999, "UMa");

    fprintf('\nScheduler diagnostic: %.2f s, %d gNB, %d UE\n\n', ...
        simTime, numGNB, numUE);

    %% ---- object graph, identical in structure to phase5_Pipeline -------
    rng(cfg.seed);
