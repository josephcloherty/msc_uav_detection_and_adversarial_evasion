...
%phase5_ValidateLOS Unit test of the spatially-consistent LOS state (D5.1).
%
...
%        interpolation of independent normals fails this - its variance
%        swings between 0.25 at a cell centre and 1 at a node - which
%        would bias the LOS threshold test by where in the cell the UE
%        happens to sit. The unit-variance correction in
%        sampleSpatialField is what makes this pass, and this test is what
%        stops anyone removing it as redundant.
%     2. SPATIAL CORRELATION. Points much closer than the correlation
%        distance must be strongly correlated; points well beyond it must
%        be essentially independent. This is the property that makes the
%        LOS state change on a physical spatial scale rather than
%        flickering per packet.
%     3. PURITY AND DETERMINISM. Sampling the same point twice must give
%        the same answer, and rebuilding from the same seed must reproduce
%        the field exactly. Pathloss is evaluated per packet in whatever
%        order the simulator delivers them, so any state carried between
%        calls would make results order-dependent and break the fixed-seed
%        byte-identical regeneration contract.
%     4. THE STATE ACTUALLY MOVES. A UE flown across many correlation
%        distances at a mid-range LOS probability must change state. This
%        is the regression test for the defect this work exists to fix:
%        up to Phase 4 the state was drawn once from the initial positions
%        and frozen, so this count was zero by construction for every UE
%        in every run.
%     5. PROBABILITY TRACKING. Above the Table B-1 100 % LOS altitude the
%        state must latch to LOS; at a very low probability it must be
%        NLOS almost everywhere. The threshold test must follow pLOS, not
%        merely wander.
%     6. FROZEN FALLBACK. With no field present (an archived Phase 4
%        replay, or cfg.dynamicLOS false) linkState must return the
%        setup-time draw and report dynamic == false, so old bundles still
%        replay and are visibly labelled as frozen.
%
%   STATISTICAL TESTS ARE POOLED ACROSS FIELDS. A single realisation of a
%   correlated field over a 1 km square contains only a few hundred
%   independent cells, so its spatial mean and variance scatter by about
%   0.05 around the true values. Testing one realisation against a tight
%   tolerance would fail perfectly correct code roughly a third of the
%   time. Pooling 40 independent fields shrinks that scatter enough for
%   the tolerances below, every one of which was set from the observed
%   spread over 60 repeated trials rather than guessed.
