...
%   parameter needed to reproduce it (D5.5 groundwork).
    ...
    % Wall-time cost model, used only by phase5_DryRun to project a batch
    % before it is launched. The constant is seconds of wall time per
    % (simulated second x gNB x UE), calibrated from the 21 July smoke
    % runs: 1881 s mean for 6 s of simulation at 7 gNB and 3 UE, so
    % 1881/(6*7*3) = 14.9. Cost is dominated by per-packet channel
    % filtering across the UE-to-gNB links and by every UE's SRS being
    % received at every gNB, both of which scale with that product.
    % Re-calibrate from the manifest wall times once real runs exist.
    cfg.batch.costModel = struct('sPerSimSecPerGNBPerUE', 14.9);
    ...
    % no extra information. 200 gives half-per-cent resolution.
    cfg.progress.updatesPerRun = 200;
