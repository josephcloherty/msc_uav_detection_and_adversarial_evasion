...
%phase5_RebuildManifest Reconstruct a batch manifest from what is on disk.
%
%   M = phase5_RebuildManifest()            uses phase5_Config
%   M = phase5_RebuildManifest(CFG)
%   M = phase5_RebuildManifest(CFG, SEEDS)  restrict to these seeds
%
%   WHY THIS EXISTS
%   The manifest is assembled on the client and written when the batch
%   returns, so a client that dies first loses the provenance record for
%   every run in the batch even though the runs are safely on disk. That
%   happened on 6 August: MATLAB's desktop render process was killed out of
%   memory late in a twenty-four run batch, and twenty-three finished runs
%   ended up with feature CSVs and no manifest.
%
%   Nothing about a run's identity is lost in that situation. The generation
%   parameters are deterministic in (scenario, seed), so they regenerate
%   exactly from the configuration; phase5_ScenarioGen is called here for
%   each run found on disk, which is the same call the batch made. The row
%   count comes from the CSV itself. The outcome fields that need the live
%   simulation, handover totals and wall time, come from the run's own
%   sidecar where phase5_RunInfo wrote one, and are NaN for runs that
%   finished before sidecars existed.
%
%   The result is written as manifest_rebuilt_<stamp>.csv, never over a
%   manifest the batch itself wrote, and returned as a table.
%
%   See also phase5_RunBatch, phase5_RunInfo, phase5_MergeDataset.
    ...
    % Driven by the files rather than by cfg.batch.seedRange, so a manifest
    % can be rebuilt across several batches at once and does not depend on
    % the configuration still holding the range that produced them.
    ...
    for i = 1:n
        ...
        if ~isempty(info)
            ...
            % No sidecar: the replay bundle still tells us whether one was
            % written, which is part of the record even when nothing else
            % about the run's execution survived.
    ...
    % Any measured wall times recovered from sidecars are worth adding to the
    % cost model, which is otherwise only updated by a batch that returns.
    try
...
%mergeInfo Prefer the sidecar's values where it has them.
...
%buildRebuiltManifest Same columns as the batch manifest, same order.
%   Deliberately a separate implementation from phase5_RunBatch's private
%   builder rather than a shared one: this file must keep working when the
%   runner changes, because its whole purpose is recovering runs made by an
%   earlier version of the runner.
    n = numel(runCfgs);
    M = table();
    M.runIdx        = (1:n)';
...
%writeCsv Same hand-rolled format as the batch manifest.
    fid = fopen(outPath, 'w');
    ...
    fprintf(fid, '%s\n', strjoin(names, ','));
    % Quote character in a variable, as elsewhere in the project: a literal
    % quote beside a closing parenthesis inside brackets is the one construct
    % MATLAB's transpose and string syntaxes share.
    q = '"';
    for r = 1:height(M)
