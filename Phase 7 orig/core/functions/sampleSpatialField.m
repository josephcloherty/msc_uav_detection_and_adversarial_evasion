...
%
%   Plain bilinear interpolation of independent normals gives variance
%   sum(w.^2), which swings between 0.25 at a cell centre and 1 at a node, so
%   the result is divided by sqrt(sum(w.^2)) to restore the unit marginal.
%
%   The autocorrelation then falls to zero at one grid cell rather than
%   decaying exponentially: the correlation distance is matched but the shape
%   is not, and that is in the deviations log. U comes from erfc rather than
%   normcdf, so no Statistics toolbox licence is needed at run time.
%
%   See also buildSpatialField, linkState.
    ...
    % Clamped to the last full cell, so a UE outside the padded grid gets
    % the boundary value rather than an error.
